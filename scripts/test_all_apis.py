...
==================================================================
Covers every endpoint group in order:

  1.  Health
  2.  Organisations     (CRUD)
  3.  Networks          (create under org, join rules, members)
  4.  Tenants           (list + detail — now includes org_id)
  5.  Users             (create, list via network_id, status, bulk-status)
  6.  Memberships       (join, suggestions, approve, reject, remove)
  7.  Profiles          (update, get, facts, patch)
  8.  Ingestion         (text ingest)
  9.  Signals           (intent signal)
  10. Graph / iKG       (upsert, get person)
  11. Matchmaking       (generate with network_id + active_users validation)
  12. Match detail      (get, explanation)
  13. Feedback          (accept, feedback)
  14. Analytics
  15. Ontology
  16. Admin             (wipe no-op)
  17. Edge cases        (duplicate slug, bad join rule, non-member match filter)

Usage:
  pip install httpx
  python test_all_apis.py
  python test_all_apis.py --url http://my-server:8000
"""
...
# MongoDB-style IDs — testing the ID normalisation layer
...
# Fresh IDs per run — prevents conflicts on repeated test runs
RUN      = uuid.uuid4().hex[:6]
ORG_ID   = None      # set after create
NET_ID   = None      # set after create
RULE_ID  = None      # set after add rule
USER_A_ID = str(uuid.uuid4())
USER_B_ID = str(uuid.uuid4())
USER_C_ID = str(uuid.uuid4())   # used for reject / remove tests
MATCH_ID  = None
DOC_ID_A  = None
...
def test_health(c: httpx.Client):
    ...
    if ok:
        for svc, status in r.json().items():
            info(f"{svc}: {status}")
...
def test_networks(c: httpx.Client):
    ...
    # POST — duplicate network slug → 409
    r = c.post(f"{BASE_URL}/v1/organisations/{ORG_ID}/networks", json={
        "name": "Dup Net",
        "slug": net_slug,
    })
    ...
    # POST — network under unknown org → 404
    ...
    # POST — duplicate email_domain rule → 409
    r = c.post(f"{BASE_URL}/v1/networks/{NET_ID}/rules", json={
        "rule_type":  "email_domain",
        "rule_value": f"testorg-{RUN}.com",
    })
    ...
    # POST — email_domain without rule_value → 400
    r = c.post(f"{BASE_URL}/v1/networks/{NET_ID}/rules", json={
        "rule_type": "email_domain",
    })
...
# ─────────────────────────────────────────────────────────────
#  5. USERS  (tenant_id removed; list via network_id)
# ─────────────────────────────────────────────────────────────
...
def test_users(c: httpx.Client):
    ...
    # Create User A (email matches org domain — will auto-join)
    ...
    ok = check(r.status_code in (200, 201), "POST /v1/users (User A)",
               f"status={r.status_code}")
    if ok:
        suggestions = r.json().get("network_suggestions", [])
        info(f"network_suggestions: {len(suggestions)} returned")
        check("network_suggestions" in r.json(),
              "POST /v1/users response includes network_suggestions")
        check("tenant_id" not in r.json(),
              "POST /v1/users response has no tenant_id field")

    # Create User B (different domain — no auto-suggestions)
    r = c.post(f"{BASE_URL}/v1/users", json={
        "user_id":      USER_B_ID,
    ...
    # Create User C (for reject/remove tests)
    r = c.post(f"{BASE_URL}/v1/users", json={
        "user_id":      USER_C_ID,
    ...
    # GET users by network_id (after memberships are set up in section 6)
    if NET_ID:
        r = c.get(f"{BASE_URL}/v1/users", params={"network_id": NET_ID})
        check(r.status_code == 200, "GET /v1/users?network_id=...",
              f"count={r.json().get('count','?')}")
        check("network_id" in r.json(), "GET /v1/users response uses network_id key")

    # PATCH user status
    r = c.patch(f"{BASE_URL}/v1/users/{USER_A_ID}/status", json={
        "status": "active",
    })
    check(r.status_code in (200, 404), "PATCH /v1/users/{id}/status",
          f"status={r.status_code}")

    # Bulk status — requires network_id now
    if NET_ID:
        ...
        check(r.status_code == 200, "POST /v1/users/bulk-status (network_id)",
              f"updated={r.json().get('users_updated','?')} skipped={r.json().get('skipped','?')}")
...
def test_memberships(c: httpx.Client):
    ...
    # GET network suggestions for User A (should see the open network)
    r = c.get(f"{BASE_URL}/v1/users/{USER_A_ID}/network-suggestions")
    ok = check(r.status_code == 200, "GET /v1/users/{id}/network-suggestions",
               f"count={r.json().get('count','?')}")
    if ok:
        info(f"suggestions: {[s['name'] for s in r.json().get('network_suggestions',[])]}")

    # User A joins (open rule → auto-approve)
    r = c.post(f"{BASE_URL}/v1/users/{USER_A_ID}/join", json={
        "network_id": NET_ID,
    })
    ...
    if ok:
        check(r.json().get("status") == "active",
              "User A join status is 'active' (auto-approved)",
              f"status={r.json().get('status','?')}")
        check(r.json().get("auto_approved") is True,
              "auto_approved=true in response")

    # User A joins again → idempotent
    r = c.post(f"{BASE_URL}/v1/users/{USER_A_ID}/join", json={
        "network_id": NET_ID,
    })
    ...
    # User B joins — explicit network (open rule still in place so auto-approved)
    r = c.post(f"{BASE_URL}/v1/users/{USER_B_ID}/join", json={
        "network_id": NET_ID,
    })
    check(r.status_code == 200, "POST /v1/users/{id}/join (User B)",
          f"status={r.json().get('status','?')}")

    # User C joins — will be rejected
    r = c.post(f"{BASE_URL}/v1/users/{USER_C_ID}/join", json={
        "network_id": NET_ID,
    })
    check(r.status_code == 200, "POST /v1/users/{id}/join (User C, for reject test)",
          f"status={r.json().get('status','?')}")

    # Unknown user join → 404
    r = c.post(f"{BASE_URL}/v1/users/{uuid.uuid4()}/join", json={
        "network_id": NET_ID,
    })
    check(r.status_code == 404, "POST join (unknown user → 404)",
          f"status={r.status_code}")

    # Unknown network join → 404
    r = c.post(f"{BASE_URL}/v1/users/{USER_A_ID}/join", json={
    ...
    # Approve User B (even if auto-approved, testing the endpoint)
    # First create an explicit-only test by rejecting User C
    ...
    # GET network members (should now have members)
    r = c.get(f"{BASE_URL}/v1/networks/{NET_ID}/members")
    ...
    # Remove User B again → 404 or 409
    r = c.delete(f"{BASE_URL}/v1/networks/{NET_ID}/members/{USER_B_ID}")
    check(r.status_code in (404, 409), "DELETE member (already removed → 404/409)",
          f"status={r.status_code}")

# ─────────────────────────────────────────────────────────────
#  7. PROFILES  (tenant_id removed; network_id optional)
# ─────────────────────────────────────────────────────────────
...
def test_profiles(c: httpx.Client):
    ...
    # POST update — no tenant_id, optional network_id
    r = c.post(f"{BASE_URL}/v1/profiles/{USER_A_ID}/update", json={
        "network_id": NET_ID,   # optional network context
        "user_profile": {
    ...
    ok = check(r.status_code in (200, 201, 404), "POST /v1/profiles/{id}/update (no tenant_id)",
               f"facts={r.json().get('facts_written','?') if r.status_code==200 else r.status_code}")
    if ok and r.status_code == 200:
        check("tenant_id" not in r.json(), "Response has no tenant_id field",
              f"keys: {list(r.json().keys())}")
        check("network_id" in r.json(), "Response includes network_id field")

    # POST update User B (no network_id at all — global profile)
    ...
    check(r.status_code in (200, 201, 404), "POST /v1/profiles/{id}/update (no network_id)",
          f"status={r.status_code}")
...
def test_ingestion(c: httpx.Client):
    ...
    r = c.post(f"{BASE_URL}/v1/ingest/text", data={
        "tenant_id":   NET_ID or "00000000-0000-0000-0000-000000000001",
        "user_id":     USER_A_ID,
        "content":     (
...
def test_signals(c: httpx.Client):
    ...
    ok = check(r.status_code == 200, "POST /v1/signals/intent (User A)",
...
# ─────────────────────────────────────────────────────────────
#  11. MATCHMAKING  (network_id; active_users validated)
# ─────────────────────────────────────────────────────────────
...
def test_matchmaking(c: httpx.Client):
    ...
    ok = check(r.status_code == 200, "POST /v1/matches/generate (network_id)",
    ...
    # Backward-compat: tenant_id alias still accepted
    r2 = c.post(f"{BASE_URL}/v1/matches/generate", json={
        "tenant_id":             nid,    # old field name → alias
        "requesting_user_id":    USER_A_ID,
        "transaction_types":     ["knowledge_transfer"],
        "max_candidates":        3,
        "min_score":             0.0,
    ...
    # Active users non-member filtered — supply a random UUID that is not a member
    ghost_id = str(uuid.uuid4())
    r3 = c.post(f"{BASE_URL}/v1/matches/generate", json={
        "network_id":            nid,
        "requesting_user_id":    USER_A_ID,
        "active_users":          [USER_A_ID, ghost_id],   # ghost should be silently filtered
        "max_candidates":        3,
        "min_score":             0.0,
        "generate_explanations": False,
    }, timeout=120)
...
def test_analytics(c: httpx.Client):
    ...
    check(r.status_code in (200, 404), "GET /v1/analytics/{network_id}/overview",
          f"status={r.status_code}")

    r = c.get(f"{BASE_URL}/v1/analytics/{nid}/match-quality")
    check(r.status_code in (200, 404), "GET /v1/analytics/{network_id}/match-quality",
          f"status={r.status_code}")
...
def test_admin(c: httpx.Client):
    ...
    # Wipe with confirm=False → 400 (safe no-op)
...
def test_edge_cases(c: httpx.Client):
    ...
    # MongoDB ObjectID format user creation
    mongo_uid = mongo_to_uuid(MONGO_ID)
    r = c.post(f"{BASE_URL}/v1/users", json={
        "user_id":      mongo_uid,
    ...
    check(r.status_code in (200, 201), "POST /v1/users (MongoDB ObjectID format)",
          f"uuid={mongo_uid[:8]}")
